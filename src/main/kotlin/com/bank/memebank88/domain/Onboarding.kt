...
data class Onboarding(
    ...
    fun verifyEmail(code: String): Either<DomainError, Onboarding> = either {
        ensure(status == OnboardingStatus.STARTED) { stepError(OnboardingStatus.STARTED) }
        ensure(code == emailCode) { DomainError.VerificationFailed("email") }
        copy(status = OnboardingStatus.EMAIL_VERIFIED)
    }

    /** Step 2: capture customer info; [token] is the freshly issued session token. */
    fun submitInfo(name: String, phone: String, token: String): Either<DomainError, Onboarding> = either {
        ensure(status == OnboardingStatus.EMAIL_VERIFIED) { stepError(OnboardingStatus.EMAIL_VERIFIED) }
        ensure(name.isNotBlank()) { DomainError.InvalidCustomerInfo("name must not be blank") }
        ensure(phone.isNotBlank()) { DomainError.InvalidCustomerInfo("phone must not be blank") }
    ...
    fun verifyToken(token: String): Either<DomainError, Onboarding> = either {
        ensure(status == OnboardingStatus.INFO_SUBMITTED) { stepError(OnboardingStatus.INFO_SUBMITTED) }
        ensure(token == sessionToken) { DomainError.VerificationFailed("token") }
        copy(status = OnboardingStatus.TOKEN_VERIFIED)
    }

    /** Step 4: record the credit decision; [accountId] is set only when [approved]. */
    fun complete(score: Int, approved: Boolean, accountId: String?): Either<DomainError, Onboarding> = either {
    ...
    private fun stepError(expected: OnboardingStatus) =
        DomainError.OnboardingStepOutOfOrder(expected.name, status.name)
}
