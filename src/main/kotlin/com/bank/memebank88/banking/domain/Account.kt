...
data class Account(
    ...
    fun debit(amount: Double): Either<BankingError, Unit> = either {
        ensure(amount > 0.00) { BankingError.InvalidAmount(amount) }
        ensure(amount <= balance) { BankingError.InsufficientFunds(id, amount, balance) }
        balance -= amount
        _domainEvents.add(AccountDebitedEvent(id, amount, Instant.now()))
    }

    fun credit(amount: Double): Either<BankingError, Unit> = either {
        ensure(amount > 0.00) { BankingError.InvalidAmount(amount) }
        balance += amount
